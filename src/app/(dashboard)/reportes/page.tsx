...
async function getReportesData() {
    ...
    // ── Clima por curso con datos reales por semana ──
    const WEEK_LABELS = ["Semana 1", "Semana 2", "Semana 3", "Semana 4"]
    const now = new Date()

    const coursesClimate = (courses ?? [])
        .filter(c => !!c.id)
        .map(c => {
            ...
            const weeks = WEEK_LABELS.map((semana, wi) => {
                ...
                weekStart.setDate(now.getDate() - (3 - wi) * 7)
            ...
            // Estudiantes del curso con última emoción
    ...
    // ── Lista de estudiantes para PDF individual ──
